...
3. Optional: profile image path (if a styled resume PDF should include a headshot)
...
5. **Build experience database structure:**
   ```json
   {
     "roles": [
       {
...
```
If URL is LinkedIn:
- Normalize to public URL pattern: https://www.linkedin.com/jobs/view/{job_id}/
- If /comm/jobs/view/... returns sign-in shell, retry with /jobs/view/{job_id}/
...
```
Preferred:
1) Styled output (recommended):
   scripts/render_pretty_resume.py \
     --input-md {resume.md} \
     --profile-image {resume_dir}/assets/profile_candidate.jpg \
     --output-pdf {resume_pretty.pdf}

2) Plain conversion:
   scripts/export_resume.py --input {resume.md} --formats pdf

3) Tool-based fallback:
   pandoc {resume.md} -o {resume.pdf} [with available PDF engine]
...
**Output:** `{Name}_{Company}_{Role}_Resume_pretty.pdf` (preferred) or `{Name}_{Company}_{Role}_Resume.pdf`
...
**Edge Case 3b: LinkedIn URL redirects to sign-in page**
```
SCENARIO: /comm/jobs/view/... URL returns Sign in shell

HANDLING:
- Extract numeric job id from URL
- Retry with public URL: https://www.linkedin.com/jobs/view/{job_id}/
- If still blocked, ask user to paste JD text directly
```
